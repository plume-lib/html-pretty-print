...
application { mainClass = "org.plumelib.htmlprettyprint.HtmlPrettyPrint" }
...
spotless {
  ...
  kotlinGradle {
    target("**/*.gradle.kts")
    // googleStyle() indents blocks by 2 spaces, as google-java-format does for Java.
...
//
// This is a method of an object rather than of the build script, because the
// configuration cache nulls out a lambda's owner when it serializes the lambda.
object JavadocFonts {
...
tasks.withType<Javadoc>().configureEach {
  ...
  doLast { JavadocFonts.removeDejaVuFontImport(javadocTask.destinationDir) }
}
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    val marker = requireJavadocMarker
